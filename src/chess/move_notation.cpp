...
bool readCastling(const char*& it, bool castles[2]) {
	// 0-0->(+,#); 0-0-0->(+,#)
...
bool readPiecePlacement(const char*& it, MoveInfo& move) {
	move.castles[0] = false;
	move.castles[1] = false;
	move.capture = false;
...
bool readMoveNotation(const char*& it, MoveInfo& move) {
	...
	move.promote = PT_NONE;
	if (readPromote(it) && (move.type != PT_PAWN || !readPromoteType(it, move.promote))) {
